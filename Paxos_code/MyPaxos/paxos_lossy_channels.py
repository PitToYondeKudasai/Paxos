...
# ----------------------------------------------------
def send_ack(s, id, i, adressees):
    s.sendto(encode(['ack', id, i]), config[adressees])
...
def send_p1b(s, sending_code, v_rnd, v_val,id):
    s.sendto(encode(['p1b', sending_code, v_rnd, v_val,id]), config['proposers'])
...
def choose_val(v_rnds, v_vals, v_id, params):
    f = lambda i: v_rnds[i]
    maximum = max(range(len(v_rnds)), key = f)
    c_val = v_vals[maximum]
    i_id = v_id[maximum]
    if (c_val != None) and (params["id"] != i_id):
...
def p1b(params):
  ...
  id=str(c_rnd).split(".")[1]
  if prop_instance in hist_list.keys():
    rnd,v_rnd,v_val,_ = hist_list[prop_instance]
    if rnd<c_rnd:
        rnd=c_rnd
    ...
    v_rnd = 0
    v_val = None
    hist_list[prop_instance]=[rnd, v_rnd, v_val,id]

  sending_code = create_sending_code(prop_instance, c_rnd)
  send_p1b(s, sending_code, v_rnd, v_val,id)
  params['hist_list'] = hist_list
  return params
...
def p2b(params):
  ...
  id=str(c_rnd).split(".")[1]
  if prop_instance in hist_list.keys():
    rnd, _,_,_ = hist_list[prop_instance]
    if c_rnd >= rnd:
      v_rnd = c_rnd
      v_val = msg_list[3]
      hist_list[prop_instance] = [c_rnd, v_rnd, v_val,id]
    else:
      params['hist_list'] = hist_list
      return params
  else:
...
def add_client_val(params, msg_list):
    if not(msg_list[2] in params['clients_values_id']):
        params["clients_values"].append(msg_list[1])
        params["clients_values_id"].append(msg_list[2])
    send_ack(params['socket_s'], params['id'], msg_list[2], 'clients')
    return params
  ...
  while True:
    ...
    try:
      ...
      if msg_list[0] == 'client val':
          params = add_client_val(params, msg_list)
...
def p1a(params):
  ...
  _, n_instance, id, rnd,_,_,_,_,s,_ = unpack_dict(params)
...
def waiting_p1b(params):
  ...
  v_rnds = []
  v_id = []
  _, n_instance, id, rnd,_,_,_,_,_,r = unpack_dict(params)
  ...
  if len(v_vals) >= Qa:
      params = choose_val(v_rnds, v_vals, v_id, params)
...
def p2a(params):
  _,n_instance,id,rnd,_,_,c_val,_,s,_ = unpack_dict(params)
  sending_code = create_sending_code(n_instance, rnd, id)
...
def waiting_p2b(params):
  _, n_instance, id, rnd, clients_values, clients_values_id,_, my_value,_,r = unpack_dict(params)
  c_rnd = float(str(rnd) + '.' + str(id))
  sending_code = create_sending_code(n_instance, rnd, id)
  start = time.time()
  Q = 0
...
def push_message(params):
    ...
    s = params['socket_s']
    r = params['socket_r']
    while len(list_ack) < n_proposers:

        i_id = float(str(i) + '.' + str(id))
        msg = encode(['client val', value, i_id ])
        s.sendto(msg, config['proposers'])
        try:
          msg = r.recv(msg_length)
          msg_list = decode(msg)
          if (msg_list[0] == 'ack') and (not (msg_list[1] in list_ack)) and (msg_list[2] == i_id):
              list_ack.append(msg_list[1])
        except socket.timeout:
          continue
    params['i'] +=  1
    return params
...
def send_decision(params):
  _, n_instance, id, rnd, _,_, v_val, _,s,r = unpack_dict(params)
  list_ack = []
  sending_code = create_sending_code(n_instance, rnd, id)
  i_id = float(str(n_instance)+"."+str(id))
  while len(list_ack) < n_learners:
      ...
      try:
        ...
        if msg_list[0] == 'client val':
            params = add_client_val(params, msg_list)
        elif (msg_list[0] == 'ack') and (not (msg_list[1] in list_ack)) and (msg_list[2] == i_id):
            list_ack.append(msg_list[1])
      except socket.timeout:
        continue
...
def decision_to_list(params,msg_list):
  ...
  if not(n_instance in params['decisions']):
    params["decisions"][int(n_instance)] = val
  i_id=float(str(n_instance)+"."+str(c_rnd).split(".")[1])
  send_ack(params['socket_s'], params['id'],i_id, 'proposers')
  return(params)
...
def proposer(config, id):
  ...
  if (id == 1):
    while True:
      ...
      #print(id, len(params["clients_values_id"]))
...
def client(config, id):
  print '-> client ', id
  r = mcast_receiver(config['clients'])
